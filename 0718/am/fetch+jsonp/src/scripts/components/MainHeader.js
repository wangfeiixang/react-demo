...
class MainHeader extends React.Component {
	...
	getHotSearch(props){
		var that=this
		$.ajax({
			url:'http://localhost:9000/shopping/v3/hot_search_words?latitude='+props.position.latitude+'&longitude='+props.position.longitude,
	...
	getWeather(props){
		var that=this;
		$.ajax({
			url:'http://localhost:9000/bgs/weather/current?latitude='+props.position.latitude+'&longitude='+props.position.longitude,
			success:function(result){
				that.setState({
